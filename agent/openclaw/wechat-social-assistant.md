...
OpenClaw can use `wechat-social-assistant` without a native plugin by treating `wsa` as a local CLI-backed agent capability.
...
- Use read-only commands for exploration: `contacts`, `brief`, `next`, `suggest`, and `profiles`.
