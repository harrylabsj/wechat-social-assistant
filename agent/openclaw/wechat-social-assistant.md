...
- Use read-only commands for exploration: `audit`, `dashboard`, `contacts`, `brief`, `quality`, `candidates`, `sources`, `weekly-report`, `feedback-list`, `next`, `suggest`, and `profiles`.
...
- Ask for explicit user confirmation before commands that capture screenshots, import images, import local relationship source files, export/delete local data, import/export Obsidian notes, write reports, reset memory, start watch mode, or stop processes.
...
```text
Use the local wechat-social-assistant CLI. Start with `wsa status` and `wsa audit`. Do not read WeChat private databases or send messages. Ask before running capture, watch, import-image, import-source, export-data, delete-contact, import-obsidian, analyze, export-obsidian, reset, or stop-watch.
```
